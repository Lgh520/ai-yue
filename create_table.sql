...
CREATE TABLE aiyue.`consige_address` (
  `CONSIGN_ID` int(11) NOT NULL COMMENT '主键，自增ID',
...
CREATE TABLE aiyue.`book_info` (
  `BOOK_ID` bigint NOT NULL COMMENT '主键，自增ID',
...
CREATE TABLE aiyue.`book_rent` (
  `RENT_ID` bigint NOT NULL COMMENT '主键，自增ID',
  `USER_ID` varchar(16) DEFAULT NULL  COMMENT '用户ID（手机号）',
  ...
  `IS_BACK` varchar(4) DEFAULT NULL COMMENT '是否归还：0-未归还，1-已归还',
	...
	`UPDATE_TIME` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`RENT_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT '书籍借阅表';

CREATE TABLE aiyue.`delivery_record` (
  `DELIVERY_ID` bigint NOT NULL COMMENT '主键，自增ID',
